...
export function CourseNode({ data, selected }: NodeProps) {
  ...
  const getIcon = () => {
    ...
    if (isAvailable) return <BookOpen className="w-5 h-5" />
  ...
  return (
    <div
      className={cn(
        ...
        "backdrop-blur-md border rounded-xl p-4 shadow-lg hover:shadow-xl",
    ...
    >
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between mb-3">
          <div
            className={cn("flex items-center justify-center w-8 h-8 rounded-lg backdrop-blur-sm", {
      ...
      <Handle type="target" position={Position.Left} className="w-2 h-2 border border-gray-400/50 bg-white/80" />
      <Handle type="source" position={Position.Right} className="w-2 h-2 border border-gray-400/50 bg-white/80" />
    </div>
  )
}
